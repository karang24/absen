				...
				<div class="row">
					...
					<div class="ibox ">
						<div class="ibox-title">
						   <center><h4>Requisition</h4></center>
						...
						<div class="ibox-content">
											...
											<div class="modal-content">
												...
												<div class="modal-body">
													...
													<div class="form-group">
															<label>Name</label> 
															<input type="text" name="nama"  placeholder="Nama Lengkap" value="{{$karyawan[0]->nama}}" class="form-control">
															<input type="hidden" name="id_pemohon"  placeholder="Nama Lengkap" value="{{$karyawan[0]->id}}" class="form-control">
													</div>
													<div class="form-group">
															<label>NIK SAP</label> 
															<input type="number" name="nik_sap"  placeholder="Nik SAP" value="{{$karyawan[0]->nik_sap}}" class="form-control" >
													</div>
													<div class="form-group">
															<label>NO ID</label>
															<input type="text" name="no_id" placeholder="No ID" value="{{$karyawan[0]->no_id}}"  class="form-control">
													</div>
													<div class="form-group">
							...
							<div class="col-lg-12 animated fadeInRight">
									<div class="mail-box-header">					  
										<h2>
											Request ({{$data->where('status','Progres')->count()}})
										</h2>
									...
									<div class="mail-box">
										...
										<tbody>
										@foreach ($data->wherein('status',['Proses', 'Hold']) as $data1)
										@if ($data1->status =="New")
										<tr class=  "unread">
										@else 
											<tr class=  "read">
										@endif
											...
											<td class="mail-ontact"> 
												@if (session('id_role') != 1)
													...
													{{$data1->nama}}
												@endif
											@if ($data1->status == "Hold") <span class="label label-danger float-right">{{$data1->status}}</span>@endif 
											@if ($data1->status == "Proses") <span class="label label-warning float-right">{{$data1->status}}</span>@endif </td>
