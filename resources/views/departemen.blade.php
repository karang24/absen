							...
							{{-- notifikasi form validasi --}}
								...
								<div class="modal fade" id="importExcel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
									<div class="modal-dialog" role="document">
										<form method="POST" action="{{url('/departemen/add')}}" enctype="multipart/form-data">
...
$.ajaxSetup({
 ...
 $(function () {
    ...
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('data.departemen.get') }}",
        columns: [
  	...
  	$('body').on('click', '.deleteItem', function () {
		...
		Swal.fire({
		  ...
		  if (result.isConfirmed) {
			$.ajax({
					type: "DELETE",
					url: "{{url('/data/departemen/delete')}}"+'/'+ Item_id,
			...
			Swal.fire(
			  ...
			  'success'
		  			...
		  			window.location.href = "{{ url('/departemen') }}";
